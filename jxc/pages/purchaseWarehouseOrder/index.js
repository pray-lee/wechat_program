...
Page({
    ...
    formSubmit(e) {
        ...
        // 处理一下提交格式
        this.formatSubmitData(this.data.purchaseWarehouseOrderDetailList, 'billDetailList')
        this.formatSubmitData(this.data.submitData.billFilesObj, 'billFiles')
        this.addLoading()
        var url = ''
        if (this.data.type === 'add') {
            url = app.globalData.url + 'purchaseWarehousingOrderController.do?doAdd'
        } else {
            url = app.globalData.url + 'purchaseWarehousingOrderController.do?doUpdate&id=' + this.data.billId
        }
        // 处理一下 null 变成字符串的问题
    ...
    bindObjPickerChange(e) {
        ...
        if (name === 'accountbookId') {
            ...
            this.showOaProcessByBillType(this.data[listName][value].id, 105)
            // ============ 审批流 =========
    ...
    getPurchaseWarehouseOrderDetailFromStorage() {
        ...
        wx.getStorage({
            ...
            success: res => {
                const purchaseWarehouseOrderDetail = res.data
                if (!!purchaseWarehouseOrderDetail) {
                    ...
                    if (!!index || index === 0) {
                        ...
                        this.setData({
                            purchaseWarehouseOrderDetailList: purchaseWarehouseOrderDetailList
                        })
                    } else {
                        this.setData({
                            purchaseWarehouseOrderDetailList: purchaseWarehouseOrderDetailList.concat(purchaseWarehouseOrderDetail)
                        })
                    }
    ...
    onShow() {
        ...
        this.getPurchaseWarehouseOrderDetailFromStorage()
    },
    ...
    deletePurchaseWarehouseOrderDetail(e) {
        var idx = e.currentTarget.dataset.index
        var purchaseWarehouseOrderDetailList = this.data.purchaseWarehouseOrderDetailList.filter((item, index) => {
            return idx !== index
        })
    ...
    getProcess(fields) {
        const params = this.getOaParams(fields, 105)
        this.addLoading()
        request({
            hideLoading: this.hideLoading,
            ...
            success: res => {
                (async () => {
                    if (res.data.success && res.data.obj.length) {
                        ...
                        this.showOaProcessByBillType(accountbookId, 105)
                        // ============ 审批流 =========
                        // ============ 外币 =========
    ...
    onAddPurchaseWarehouseOrderDetail() {
        var obj = this.generateBaseDetail()
        wx.setStorage({
            key: 'initPurchaseWarehouseOrderDetail',
            data: obj,
            success: res => {
                console.log('写入报销详情成功...')
                wx.navigateTo({
                    url: '/jxc/pages/purchaseWarehouseOrderDetail/index'
                })
            }
        })
    },
    ...
    getEditData(id) {
        ...
        request({
            hideLoading: this.hideLoading,
            url: app.globalData.url + 'purchaseWarehousingOrderController.do?getPurchaseWarehousingOrder&id=' + id,
            method: 'GET',
            dataType: 'json',
            success: res => {
    ...
    showPurchaseWarehouseOrderDetail(e) {
        // 加一个编辑标志
        wx.setStorage({
            key: 'edit',
            data: true,
        ...
        this.addLoading()
        const index = e.currentTarget.dataset.index
        this.data.purchaseWarehouseOrderDetailList[index].taxpayerType = this.data.submitData.taxpayerType
        this.data.purchaseWarehouseOrderDetailList[index].taxRageObject = this.data.taxRageObject
        var obj = this.generateBaseDetail()
        wx.setStorage({
            key: 'index',
            data: index,
        ...
        wx.setStorage({
            ...
            data: this.data.purchaseWarehouseOrderDetailList[index],
            success: res => {
                console.log(this.data.purchaseWarehouseOrderDetailList[index])
                console.log('写入采购入库单详情成功！！')
                wx.setStorage({
                    key: 'initPurchaseWarehouseOrderDetail',
                    data: obj,
                    success: res => {
        ...
        wx.showModal({
            ...
            success: res => {
                if (res.confirm) {
                    ...
                    request({
                        hideLoading: this.hideLoading,
                        url: app.globalData.url + 'purchaseWarehousingOrderController.do?doBatchDel&ids=' + this.data.billId,
                        method: 'GET',
                        success: res => {
                            console.log(res)
                            if (res.data.success) {
    ...
    getProcessInstance(billId, accountbookId) {
        ...
        request({
            hideLoading: this.hideLoading,
            url: app.globalData.url + 'weixinController.do?getProcessinstanceJson&billType=105&billId=' + billId + '&accountbookId=' + accountbookId,
            method: 'GET',
            success: res => {
                console.log(res, '审批流')
                if (res.data && res.data.length) {
    ...
    async initCurrency(accountbookId, currencyTypeId, exchangeRate, data) {
        ...
        if (multiCurrency) {
            ...
            if (data && data.billDetailList && data.billDetailList.length) {
                ...
                this.setData({
                    purchaseWarehouseOrderDetailList: billDetailList,
                })
            }
        } else {
